...
// che viene saltata dal client"): posizioniMappa è per l'editor, non
...
class ManifestTest {
    ...
    fun senzaPosizioniMappaIlDefaultEVuoto() {
        assertTrue(manifestMinimo().posizioniMappa.isEmpty())
    }

    @Test
    fun posizioniMappaSopravviveAUnGiroDiSerializzazione() {
        val originale = manifestMinimo().copy(
            posizioniMappa = mapOf("1" to PosizioneScena(10f, 20f), "2" to PosizioneScena(-5f, 300f)),
        )
        ...
        assertEquals(originale.posizioniMappa, decodificato.posizioniMappa)
    }
    ...
    fun unJsonSenzaIlCampoPosizioniMappaRestaValido() {
        ...
        assertTrue(manifest.posizioniMappa.isEmpty())
    }
}
